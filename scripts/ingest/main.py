...
df = pd.read_excel(filepath, sheet_name='Arup Statements v2')
df = df[df['Option 1'].notna()]
...
for index, row in df.iterrows():
    ...
    for i in range(1, 4):
        ...
        choices[str(i)] = Choice(
            ...
            outcome=Outcome(
                description=str(row[result_col]) if result_col in row and pd.notna(row[result_col]) else ""
            )
        )
