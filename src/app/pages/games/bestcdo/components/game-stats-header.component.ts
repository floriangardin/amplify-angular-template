...
export class GameStatsHeaderComponent {
  ...
  dynamicMessage = computed<DynamicMessage>(() => {
    ...
    // Priority 2: Last email was a budget email (triggered by high client relationship)
    if (email?.category === 'budget') {
      return { text: '💰 You received budget because your client relationship is high', color: 'green' };
    }
    ...
    if (crPct < 80) {
      candidates.push({ key: 'cr', pct: crPct, msg: { text: '⭐ Improve your client relationship to get more budget', color: 'orange' } });
    }
    if (dqPct < 50) {
    ...
    if (budgetPct < 20) { // 200k / 1e6 = 20%
      candidates.push({ key: 'budget', pct: budgetPct, msg: { text: '💰 Budget is very low, consider improving client relationship', color: 'orange' } });
    }
