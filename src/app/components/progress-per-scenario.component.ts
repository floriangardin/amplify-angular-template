  ...
  template: `
      ...
      <ng-container>
        @for (item of viewModels(); track item.scenario.nameId) {
          <div (click)="clickedScenarioLeaderboard.emit(item.scenario)" class="bg-black/20 hover:bg-black/10 text-white rounded-lg px-4 py-3 flex items-center justify-between shadow border border-black/30 cursor-pointer">
            <div class="min-w-0">
...
export class ProgressPerScenarioComponent {
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      for (const s of scenarios) {
        ...
        if (!row?.done || row?.rank != null) continue;
  ...
  private async loadRankForScenario(s: Scenario, opts?: { isCancelled?: () => boolean }) {
    try {
      ...
      map[nameId] = { ...current, rank, trophy, profit, ...medal };
      this.byScenario.set(map);
    } catch (err) {
